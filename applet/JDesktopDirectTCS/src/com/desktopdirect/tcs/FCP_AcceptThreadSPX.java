...
public FCP_AcceptThreadSPX() { } //do nothing

public FCP_AcceptThreadSPX(FatClientProxy my_owner, String ip, ServerSocket sock, int port)
{
	try {
	...
	while (true) {
		try {
			...
			print_message("Got a connection on "+this.listen_port+"\n", FatClientProxy.INFO_NOTICE);
                        ...
                        owner.winredir_connect_spx(owner.bytes2int(InetAddress.getByName(owner.LOCALHOST_ADDR).getAddress(), 0, 4),owner.INTERNAL_PORT, c, winredir_entry.encode().length, winredir_entry.encode());
                        
			client = new FCP_ClientThreadSPX(this, client_sock, c);
			...
			if (client != null && client.remote_port > 0) {
				...
				client.start();
				System.out.println("Started a connection on ");
			} else {
				System.out.println("Unable to accept a connection on \n");
			}
		} catch (Exception e) {
...
public synchronized void setEntry(FCP_WinRedirEntrySPX prmEntry) {
...
public void mark_client_ready(int remote_port, boolean ready)
{
	...
	client = (FCP_ClientThreadSPX)this.clients.get(new Integer(remote_port));
	if (client != null) {
...
public FCP_ClientThreadSPX get_client(int remote_port) {
	FCP_ClientThreadSPX client;
...
public synchronized void remove(FCP_ClientThreadSPX client)
{
	try {
	...
	if (!this.terminated) {
		...
		try {
			...
			for (e = this.clients.keys(); e.hasMoreElements(); ) {
				client = (FCP_ClientThreadSPX)this.clients.get(e.nextElement());
				client.terminate(); //calls remove()
			}
		} catch (Exception ignored) {
		}
		this.owner.removeSPX(this);
...
public boolean equals(FCP_AcceptThreadSPX accept)
{
	return (accept != null &&
