...
public class FatClientProxySPX extends FatClientProxy implements Runnable {
	...
	public FatClientProxySPX(String prmServer, int prmPort, String prmSessionID,
			String prmSessionCookie, String prmModID) {
		
		super(prmServer, prmPort, prmSessionID, prmSessionCookie, prmModID);
	...
	public SSLSocket init_control_connection()
{
	...
	int status = 0;
	  ...
	  sc.init(null, trustAllCerts, new java.security.SecureRandom());
          ...
          System.out.println("init_control_connection - server_name="+server_name+", server_port="+server_port);
	...
	public void run() {
                            ...
                            try {
                                    if (!conn_alive) {
                                            ...
                                            if (starttime == 0) {
                                                    ...
                                                    starttime = (int) (time / 1000); // convert miliseconds
                                                                                                                            // to seconds
                                            }

                                            // open a connection to the SP on the control port
                                            //sslsock = init_control_connection();
                                            sslsock = (SSLSocket) AccessController.doPrivileged(
                                            ...
                                            /*
                                             * String cookies = ""; //attempt to extract all SP set
                                             * cookies from the browser try { cookies =
                                             * (String)win.call("get_cookies",null); } catch (Exception
                                             * ignored) { } if (cookies == null) { cookies = ""; }
                                             * cookies.replace('\r', ' '); cookies.replace('\n', ' ');
                                             */

                                            // send the request to initiate the clientapp control
                                            // connection
                                            ...
                                            sessid_array = str2bytes(session_id.substring(session_id
                                                            .indexOf("+") + 1));
                                    ...
                                    case GET_TIME_INFO:
                                            if (m_GetTimersResult != null) {
                                                    ...
                                                    m_GetTimersResult
                                                                    .write(int2bytes(data_length, 4), 0, 4);
                                                    data_length = 0;
                                            }
                                    ...
                                    case GET_USERNAME:
                                            ...
                                            if (m_GetUsernameResult != null) {
                                                    m_GetUsernameResult.write(resSt.getBytes(), 0,
                                                                    resSt.length());
                                    ...
                                    case GET_PASSWORD:
                                            ...
                                            if (m_GetPasswordResult != null) {
                                                    m_GetPasswordResult.write(resSt.getBytes(), 0,
                                                                    resSt.length());
                                    ...
                                    case SEND_DATA:
                                            conn = (FCP_AcceptThreadSPX) m_Conns.get(new Integer(
                                                            remote_port));
                                            if (conn != null) {

                                                    // !!! Start
                                                    FCP_ClientThreadSPX cl = conn.get_client(remote_port);
                                                    // !!! End
                                    ...
                                    case WINREDIR_ADDRESS:
                                            ...
                                            print_message(
                                                            "Resolved "
                                                                            + bytes2str(domain, 0, data_length)
                                                                            + "to "
                                                                            + new String(((int) ip_arr[0] & 0xFF) + "."
                                                                                            + ((int) ip_arr[1] & 0xFF) + "."
                                                                                            + ((int) ip_arr[2] & 0xFF) + "."
                                                                                            + ((int) ip_arr[3] & 0xFF))
                                                                            + " for Query ID " + listen_port
                                                                            + " SPI port " + remote_port + "\n",
                                                            INFO_NOTICE);
                                            // overload listen_port as the query id
                                            break;

                                    case VALIDATE_VERSION:
                                            generic_send(IDENTIFY_MODULE, 0, 0, 0, mod_id.length(),
                                                            mod_id.getBytes());
                                            m_Ready = true;
                                            break;
            ...
            //Logger.getLogger(FatClientProxy.class.getName()).log(Level.SEVERE, null, ex);
        }
	...
	public synchronized int addConn(String prmDestIP, int prmDestPort,
		...
		FCP_AcceptThreadSPX conn = new FCP_AcceptThreadSPX(this, LOCALHOST_ADDR, null, 0);
		...
		if (conn.getListenPort() > 0) {
			...
			try {
				...
				FCP_WinRedirEntrySPX entry = new FCP_WinRedirEntrySPX(0, prmDestPort,
						bytes2int(
	...
	public synchronized int addOtherConnSPX(String prmDestIP, int prmDestPort,
		...
		if (conn.getListenPort() > 0) {
			System.out.println("Listen port is ok");
			try {
				print_message("Connecting to result pipe \n", INFO_NOTICE);
				m_AddConnResultIn.connect(m_AddConnResultOut);
				print_message("Creating WinRedirEntry \n", INFO_NOTICE);
				...
				System.out.println("Connecting LocalPort"+e.getMessage());
			}
		}
		return -1;
	}
	...
	public synchronized void sendLog(int prmLogLevel, String prmProtocol, int prmDest, String prmDestName, String prmMsg) {
		...
		try {
			...
			buf.write(str2bytes(prmProtocol));
	...
	public synchronized void remove(FCP_AcceptThreadSPX listener) {
		Hashtable listeners;
		try {
			/*
	...
	public synchronized void terminate() {
		...
		try {
			...
			System.out.println("socket send spx terminate");
			socket.close();
